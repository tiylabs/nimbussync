    ...
    (0..file_size.div_ceil(chunk_size))
        .map(|index| {
            ...
            ChunkDescriptor {
                index: index as u32,
                offset,
                length: (file_size - offset).min(chunk_size),
            }
        })
        .collect()
}
...
pub fn recover_upload(
    ...
    if session.fingerprint != *fingerprint {
        return UploadRecoveryDecision::AbandonSourceChanged;
    }
    if session
        .expires_at
        ...
        .unwrap_or(false)
    {
        return UploadRecoveryDecision::AbandonExpired;
    }
    if session.parts.iter().all(|part| part.state == "completed") {
        return UploadRecoveryDecision::VerifyCompletion;
    }
    UploadRecoveryDecision::ResumePending {
        parts: session.pending_parts().cloned().collect(),
    }
}
...
pub fn encrypted_chunks(
    ...
    chunks
        ...
        .map(|chunk| {
            ...
            let end = start
                .saturating_add(chunk.length as usize)
                .min(plaintext.len());
...
mod tests {
    ...
    fn absolute_ctr_offset_matches_whole_file_encryption() {
        ...
        let chunks = encrypted_chunks(
            ...
            &iv,
            plaintext,
            &plan_chunks(plaintext.len() as u64, 7),
        );
    ...
    fn recovery_rejects_changed_source_and_resumes_only_pending_parts() {
        ...
        let session = UploadSessionState {
            ...
            remote_session_ref: "session".into(),
        ...
        assert_eq!(
            recover_upload(&session, &fingerprint, 1_000),
            UploadRecoveryDecision::ResumePending {
                parts: vec![session.parts[1].clone()]
            }
        );
        assert_eq!(
            recover_upload(
                &session,
                &SourceFingerprint::from_bytes(b"changed", None, 3),
                1_000
            ),
            UploadRecoveryDecision::AbandonSourceChanged
        );
        assert_eq!(
            recover_upload(&session, &fingerprint, 2_000),
            UploadRecoveryDecision::AbandonExpired
        );
    }
    ...
    fn provider_matrix_fails_closed_for_unknown_or_incomplete_capabilities() {
        ...
        snapshot.providers.push(ProviderCapability {
            ...
            write: CapabilityState::Verified,
            resumable: CapabilityState::Verified,
            zero_byte: CapabilityState::Unverified,
            crypto: CapabilityState::Verified,
        });
