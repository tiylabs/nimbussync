use crate::CoreError;
use bytes::Bytes;
use cloudreve_protocol::{
    normalize_origin, ApiEnvelope, FileEventDto, FileUrlDto, RemoteFileDto, RemoteItem,
    RemoteListDto, ServerEvent, SseEvent, SseParser, UploadCredentialDto, UserDto,
};
...
impl CloudreveHttpClient {
    pub fn new(
        ...
        let normalized = normalize_origin(origin, allow_loopback_http)
            .map_err(|_| CoreError::UnsupportedServer)?;
        let mut base_url =
            Url::parse(&format!("{normalized}/")).map_err(|_| CoreError::UnsupportedServer)?;
    ...
    fn api_url(&self, path: &str) -> Result<Url, CoreError> {
        self.base_url
            .join(&format!("{API_PREFIX}/{}", path.trim_start_matches('/')))
            .map_err(|_| CoreError::UnsupportedServer)
    }
    ...
    async fn request_empty(
        ...
        decode_empty_response(builder.send().await.map_err(|_| CoreError::Network)?).await
    }

    pub async fn validate_site(&self) -> Result<cloudreve_protocol::SiteConfigDto, CoreError> {
        let version: String = self
            .request(Method::GET, "site/ping", &[], None, false)
            .await?;
        Ok(cloudreve_protocol::SiteConfigDto {
    ...
    pub async fn user_me(&self) -> Result<UserDto, CoreError> {
        self.request(Method::GET, "user/me", &[], None, true).await
    }

    pub async fn get_item_by_id(&self, id: &str) -> Result<RemoteItem, CoreError> {
        let file: RemoteFileDto = self
            .request(
                Method::GET,
                "file/info",
                &[("id", id.into()), ("extended", "true".into())],
                None,
                true,
            )
            .await?;
    ...
    pub async fn get_item_by_uri(&self, uri: &str) -> Result<RemoteItem, CoreError> {
        let file: RemoteFileDto = self
            .request(
                Method::GET,
                "file/info",
                &[("uri", uri.into()), ("extended", "true".into())],
                None,
                true,
            )
            .await?;
    ...
    pub async fn list_children_page(
        ...
        page_size: i32,
    ) -> Result<RemoteListDto, CoreError> {
        let mut query = vec![
            ("uri", uri.to_owned()),
        ...
        if let Some(next_token) = next_token {
            query.push(("next_page_token", next_token.to_owned()));
        }
        self.request(
            Method::GET,
            "file",
    ...
    pub async fn list_children_all(
        ...
        uri: &str,
        page_size: i32,
    ) -> Result<Vec<RemoteItem>, CoreError> {
        let mut page = None;
        let mut token = None;
        let mut output = Vec::new();
        loop {
            let response = self
                .list_children_page(uri, page, token.as_deref(), page_size)
                .await?;
            ...
            output.extend(
                response
                    .files
                    .into_iter()
                    .map(|file| file.to_remote_item(None)),
            );
            ...
            if let Some(total) = response.pagination.total_items {
                ...
                if current_page * size < total && count > 0 {
                    page = Some(current_page as i32 + 1);
                    token = None;
                    continue;
                }
            }
    ...
    pub async fn fetch_contents_to<W: AsyncWrite + Unpin>(
        &self,
        item_uri: &str,
        writer: &mut W,
    ) -> Result<u64, CoreError> {
        let urls: FileUrlDto = self
            .request(
                Method::POST,
                "file/url",
                &[],
                Some(json!({ "uris": [item_uri], "download": true, "redirect": false })),
                true,
            )
            .await?;
        let target = urls
        ...
        if target.scheme() != "https" || target.username() != "" || target.password().is_some() {
            return Err(CoreError::UnsupportedServer);
        }
        let response = self
            .http
            .get(target)
    ...
    pub async fn create_item(&self, uri: &str, kind: &str) -> Result<RemoteItem, CoreError> {
        let file: RemoteFileDto = self
            .request(
                ...
                "file/create",
                &[],
                Some(json!({ "uri": uri, "type": kind, "err_on_conflict": true })),
                true,
            )
            .await?;
        Ok(file.to_remote_item(None))
    ...
    pub async fn update_content(
        ...
        if let Some(previous) = previous {
            query.push(("previous", previous.to_owned()));
        }
    ...
    pub async fn rename_item(&self, uri: &str, name: &str) -> Result<RemoteItem, CoreError> {
        let file: RemoteFileDto = self
            .request(
                ...
                "file/rename",
                &[],
                Some(json!({ "uri": uri, "new_name": name })),
                true,
            )
            .await?;
        Ok(file.to_remote_item(None))
    ...
    pub async fn move_item(
        ...
        self.request_empty(
            ...
            "file/move",
            &[],
            Some(json!({ "uris": [uri], "dst": destination, "copy": copy })),
            true,
        )
        .await
    }
    ...
    pub async fn trash_item(&self, uri: &str) -> Result<(), CoreError> {
        self.request_empty(
            ...
            "file",
            &[],
            Some(json!({ "uris": [uri], "skip_soft_delete": false })),
            true,
        )
        .await
    }
    ...
    pub async fn delete_item(&self, uri: &str, recursive: bool) -> Result<(), CoreError> {
        self.request_empty(
            ...
            "file",
            &[],
            Some(json!({ "uris": [uri], "skip_soft_delete": true, "recursive": recursive })),
            true,
        )
        .await
    }
    ...
    pub async fn create_upload_session(
        ...
        uri: &str,
        size: u64,
    ) -> Result<UploadCredentialDto, CoreError> {
        self.create_upload_session_with_previous(uri, size, None)
            .await
    }

    pub async fn create_upload_session_with_previous(
        &self,
        uri: &str,
        size: u64,
        previous: Option<&str>,
    ) -> Result<UploadCredentialDto, CoreError> {
        let mut body = json!({ "uri": uri, "size": size, "policy_id": "" });
        if let Some(previous) = previous {
            body["previous"] = serde_json::Value::String(previous.to_owned());
        }
        self.request(Method::PUT, "file/upload", &[], Some(body), true)
            .await
    }
    ...
    pub async fn complete_upload(
        ...
        for value in [policy, session_id, secret] {
            if value.is_empty()
                || value.len() > 4096
                || value.contains('/')
                || value.contains('\\')
        ...
        let method = if policy.eq_ignore_ascii_case("onedrive") {
            Method::POST
        } else {
            Method::GET
        };
        self.request_empty(
            method,
            &format!("callback/{policy}/{session_id}/{secret}"),
            &[],
            None,
            false,
        )
    ...
    pub async fn subscribe_events(
        ...
        uri: &str,
        client_id: &str,
    ) -> Result<SseSubscription, CoreError> {
    ...
    pub async fn next_event(&mut self) -> Result<Option<ServerEvent>, CoreError> {
        ...
        loop {
            let Some(chunk) = self.stream.next().await else {
                self.pending
                    .extend(self.parser.finish().map_err(|_| CoreError::Network)?);
...
fn map_sse_event(event: SseEvent) -> ServerEvent {
    match event.event.as_deref() {
        ...
        Some("event") => serde_json::from_str::<Vec<FileEventDto>>(&event.data)
            .map(ServerEvent::Files)
            .or_else(|_| {
                serde_json::from_str::<FileEventDto>(&event.data)
                    .map(|value| ServerEvent::Files(vec![value]))
            })
...
async fn decode_response<T: DeserializeOwned>(response: reqwest::Response) -> Result<T, CoreError> {
    ...
    let body = response
        ...
        .await
        .map_err(|_| CoreError::Network)?;
    if status == StatusCode::UNAUTHORIZED || matches!(body.code, 401 | 40020 | 40089) {
        return Err(CoreError::Authentication);
    }
...
async fn decode_empty_response(response: reqwest::Response) -> Result<(), CoreError> {
    let status = response.status();
    let body = response
    ...
    body.into_unit().map_err(|error| map_api_error(error.code))
}
