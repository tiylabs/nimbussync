...
public final class OAuthCoordinator: @unchecked Sendable {
    ...
    public init(redirectScheme: String = "nimbussync-macos", redirectHost: String = "callback", redirectPath: String = "/desktop") { self.redirectScheme = redirectScheme; self.redirectHost = redirectHost; self.redirectPath = redirectPath }
...
public final class OAuthAuthorizationSession: NSObject, ASWebAuthenticationPresentationContextProviding, @unchecked Sendable {
    private let coordinator: OAuthCoordinator
    private let configuration: CloudreveOAuthConfiguration
    private var session: ASWebAuthenticationSession?
    private let lock = NSLock()

    public init(coordinator: OAuthCoordinator = OAuthCoordinator(), configuration: CloudreveOAuthConfiguration = CloudreveOAuthConfiguration()) {
        self.coordinator = coordinator; self.configuration = configuration
    }

    public func authorize(origin: URL) async throws -> OAuthAuthorizationResult {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let webSession = ASWebAuthenticationSession(url: url, callbackURLScheme: coordinator.redirectScheme) { [weak self] callbackURL, error in
                if let error { continuation.resume(throwing: error); return }
                do {
                    guard let self else { throw CoreFailure(code: .authentication, retryable: false, userActionRequired: true) }
                    guard let callbackURL else { throw CoreFailure(code: .authentication, retryable: false, userActionRequired: true) }
                    continuation.resume(returning: OAuthAuthorizationResult(callback: try self.coordinator.validate(url: callbackURL), verifier: transaction.verifier))
                } catch { continuation.resume(throwing: error) }
            }
            webSession.presentationContextProvider = self
            webSession.prefersEphemeralWebBrowserSession = false
            lock.lock(); session = webSession; lock.unlock()
            guard webSession.start() else {
                lock.lock(); session = nil; lock.unlock()
                continuation.resume(throwing: CoreFailure(code: .authentication, retryable: false, userActionRequired: true))
                return
            }
        }
    }

    public func presentationAnchor(for session: ASWebAuthenticationSession) -> ASPresentationAnchor { NSApplication.shared.keyWindow ?? NSApplication.shared.mainWindow ?? NSWindow() }

    public func cancel() { lock.lock(); session?.cancel(); session = nil; lock.unlock() }
}
...
public final class OAuthTokenExchange: @unchecked Sendable {
    ...
    public func exchange(origin: URL, callback: OAuthCallback, verifier: String, now: Date = Date()) async throws -> Credential {
        ...
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw CoreFailure(code: .authentication, retryable: false, userActionRequired: true) }
