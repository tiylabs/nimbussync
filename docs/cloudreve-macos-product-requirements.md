...
| 多存储 Provider 上传 | 已实现 | Local、S3、OneDrive、七牛、又拍等 | P0 |
...
| FR-TSK-008 | 支持取消和重试 | P1 | 操作结果立即反映到状态列表 |
...
File Provider Domain 是远端文件树的系统视图，不能完全复制普通目录 watcher 的 ignore 语义。目标产品将原版“忽略规则”定义为**排除展示与同步规则**。
